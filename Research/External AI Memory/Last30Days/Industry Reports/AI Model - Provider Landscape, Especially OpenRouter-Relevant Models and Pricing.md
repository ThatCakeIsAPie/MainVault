...
- **Batch inference is a volatile opportunistic tier, not durable infrastructure:** OpenRouter added 28 discounted `:batch` variants on July 29 and removed all 28 by August 1. Latency-tolerant work may use batch only behind same-run availability preflights and a paid non-batch fallback.
...
- **Native DeepSeek V4 peak/off-peak billing is live** (peak 01:00–04:00 and 06:00–10:00 UTC). Official V4-Pro is $1.32/$3.96 peak and $0.66/$1.98 off-peak. As of 2026-08-17 the OpenRouter **API list** for `deepseek/deepseek-v4-pro-0813` is $0.66/$1.98 while the model-page provider table still shows $1.32/$3.96-class rows. Production `deepseek/deepseek-v3.2` remains a different ID. Receipts must log provider, not just model ID.
- **The router layer itself is now M&A bait:** Bloomberg/TechCrunch (Aug 16) say Stripe finalized a >$7B OpenRouter deal. Stripe has not confirmed. Treat as continuity risk, not a routing change, until there is an official statement.
...
## Faleth Relevance
- Maintain a Faleth/OpenRouter routing policy: cheap model for classification/extraction (Luna / Laguna XS), preflighted batch only for disposable latency-tolerant work with a paid fallback, mid-tier for drafting/orchestration (Terra A/B vs Sonnet 5 / GPT-5.5), premium for final reasoning/review (Sol / GPT-5.5 / Sonnet 5), multimodal only when required.
...
### 2026-08-03
- Official API **~11:00 UTC**: **337 IDs**, exact diff **0 additions / 0 removals**. Core stack pricing remains unchanged: Sonnet 5 **$2/$10/M** (cache **$0.20**), GPT-5.5 **$5/$30/M** (cache **$0.50**), DeepSeek V3.2 **$0.269/$0.40/M** (cache **$0.1345**), and Laguna XS 2.1 **$0.06/$0.12/M** (cache **$0.03**) plus `:free`. Qwen3.8-Max launch coverage produced no OpenRouter ID at snapshot time; targeted Reddit reports of DeepSeek V4 Flash reasoning-effort inconsistencies reinforce provider-level assertions before routing changes ([API](https://openrouter.ai/api/v1/models), [Reuters Qwen item](https://news.google.?oc=5), [Reddit test](https://www.reddit.com/r/DeepSeek/comments/1vdqjwr/openrouter_reasoning_effort_levels_are_broken_for/)). Signal: **strong** for API continuity, **medium–weak** for provider behavior.
...
### 2026-08-17
- Official API **~11:00 UTC**: **414 IDs**, exact diff **+1 / −0**. Added `z-ai/glm-5.2:free` ($0/$0; 128K). Core/delegate stack unchanged; production V3.2 still $0.269/$0.40. OpenRouter API list for `deepseek/deepseek-v4-pro-0813` is now $0.66/$1.98 (off-peak official); page still shows $1.32/$3.96 provider rows. Bloomberg/TechCrunch Stripe >$7B OpenRouter deal is **unconfirmed** (Stripe no comment). Keep routes; log provider; do not bench GLM free without a paid fallback ([API](https://openrouter.ai/api/v1/models), [DeepSeek pricing](https://api-docs.deepseek.com/quick_start/pricing/), [OpenRouter V4-Pro-0813](https://openrouter.ai/deepseek/deepseek-v4-pro-0813), [TechCrunch](https://techcrunch.com/2026/08/16/stripe-will-reportedly-acquire-ai-gateway-startup-openrouter-for-7b/)). Signal: **strong** for API/pricing; **medium** for the Stripe rumor.
